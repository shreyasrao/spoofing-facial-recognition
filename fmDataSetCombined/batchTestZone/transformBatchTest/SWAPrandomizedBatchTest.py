...
for tol in range(45,46):
		...
		for file in files:
			if "jp" in file:
				...
				if f == s[1]:
					...
					break
	print(f)	
	enrolledImg = 'smileFaces/'+f 
	swap = rand[randint(0, 99)].rstrip()
    
    for _root,_dirs,files in os.walk("neutralFaces/"):
		for file in files:
			if "jp" in file:
				s = file.split("-")
				if swap == s[1]:
    ...
    swapImg = "neutralFaces/" + swapImg
    subprocess.Popen(['python3', 'faceswap.py', enrolledImg,  swapImg], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    
    #produce output.jpg file
    #os.rename("output.jpg","known/enrolled.jpg")
    copyfile("output.jpg","known/enrolled.jpg")
